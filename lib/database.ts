...
export async function completeTrip(planId: string, userId: string, isServer = false): Promise<TravelPlan | null> {
  const supabase = isServer ? await getServerClient() : createClient();
...
export async function getPlanReviews(planId: string, isServer = false): Promise<UserReview[]> {
  const supabase = isServer ? await getServerClient() : createClient();
...
export async function getUserReviews(userId: string, isServer = false): Promise<UserReview[]> {
  const supabase = isServer ? await getServerClient() : createClient();
...
export async function getAverageRating(userId: string, isServer = false): Promise<{ average: number; count: number }> {
  const supabase = isServer ? await getServerClient() : createClient();
